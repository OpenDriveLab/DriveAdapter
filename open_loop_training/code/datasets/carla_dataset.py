...
class CarlaDataset(BaseDataset):
    ...
    def load_bev_seg(self, fname):
        if self.is_local:
            with open(fname, "rb") as f:
                return np.load(fname,)
        else:
            return np.frombuffer(memoryview(self.client.get(fname)), np.float32).reshape(15, 192, 192)
    ...
    def get_data_info(self, data_folder, route_index, is_current, is_previous_one):
        results = {}
        ...
        if is_current:
            ...
            results["only_ap_brake"] = current_supervision['only_ap_brake']
            ## Label
            ...
            bev_seg_label = torch.from_numpy(self.load_bev_seg(os.path.join(data_folder, "bev_seg_label", f"{str(route_index).zfill(4)}.npy"))).float()            ## Road Mask - 0, Route Mask - 1, Lane_Mask -2, broken lane - 3, 4 * vehicle - 4567, 4 * walker 8 9 10 11, green 12 13 14 15, yellow 16 17 18 19, red 20 21 22 23
            green = torch.zeros((4, 192, 192))
            green[torch.logical_and(bev_seg_label[11:]>0.3, bev_seg_label[11:]<0.4)] = 1.0
            yellow = torch.zeros((4, 192, 192))
            yellow[torch.logical_and(bev_seg_label[11:]>0.6, bev_seg_label[11:]<0.7)] = 1.0
            red = torch.zeros((4, 192, 192))
            red[bev_seg_label[11:]>0.9] = 1.0
            results["bev_seg_label"] = np.concatenate([bev_seg_label[:2], (bev_seg_label[2]==1.0).float()[np.newaxis, :, :], (bev_seg_label[2]==0.5).float()[np.newaxis, :, :], bev_seg_label[3:11], green, red, yellow], axis=0)
        if is_previous_one:
            previous_one_supervision = self.load_npy(os.path.join(data_folder, "supervision", f"{str(route_index).zfill(4)}.npy")).item()
            ## throttle, steer, brake, gear=1.0
            results["previous_one_action"] = np.concatenate([previous_one_supervision["action"], np.ones((1))])
    ...
    def prepare_train_data(self, index):
        ...
        input_dict = self.get_data_info(data_folder, route_index, is_current=True, is_previous_one=False)
        ...
        for i in prev_indexs_list[:-1][::-1]:
            ...
            input_dict = self.get_data_info(data_folder, route_index + i, is_current=False, is_previous_one=is_previous_one)
...
def union2one(full_queue_pipeline, queue):
    ...
    angular_velocity_z = queue[-1]["img_metas"].data['can_bus'][12]
    queue[-1]['img'] = DC(imgs_list,
                            cpu_only=False, stack=True)
    queue[-1]['img_metas'] = DC(metas_map, cpu_only=True)
    queue[-1]["state_list"] =  DC(torch.from_numpy(np.concatenate([queue[-2]["previous_one_action"], [np.sqrt(max(queue[-1]['speed']**2-(angular_velocity_z*1.5)**2, 0.0)), angular_velocity_z*1.5]])).float(), cpu_only=True)
    queue = queue[-1]
    return queue
